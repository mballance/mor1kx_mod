...
{
	ulong block_size = (mfspr_DCCFGR() & SPR_DCCFGR_CBS) ? 32 : 16;
...
{
	ulong block_size = (mfspr_ICCFGR() & SPR_ICCFGR_CBS) ? 32 : 16;
...
int cache_init(void)
{
	if (mfspr_UPR() & SPR_UPR_ICP) {
		icache_disable();
		invalidate_icache_range(0, checkicache());
		icache_enable();
	}

	if (mfspr_UPR() & SPR_UPR_DCP) {
		dcache_disable();
